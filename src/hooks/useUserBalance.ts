...
export function useUserBalance(
  ...
  const callsRequests: CallRequest[] = useMemo(() => {
    ...
    const hasTP = Array.isArray(contracts.TP) && Array.isArray(contracts.Moc)
    if (hasTP) {
      const caLen = tokens.CA.length
    ...
    const hasCA = Array.isArray(contracts.CA) && Array.isArray(contracts.Moc)
    if (hasCA) {
      let countRC20 = 0
      const caLen = tokens.CA.length
      for (let ca = 0; ca < caLen; ca++) {
    ...
    if (import.meta.env.REACT_APP_ENVIRONMENT_APP_PROJECT === 'voting') {
      const firstCT = Array.isArray(contracts.CollateralToken) ? contracts.CollateralToken[0] : undefined
      if (firstCT) {
        calls.push({
          contract: firstCT,
          functionName: 'balanceOf',
