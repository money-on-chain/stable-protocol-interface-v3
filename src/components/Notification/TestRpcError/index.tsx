...
export default function TestRpcError(): React.ReactElement {
    const { rpcError, handleRpcError, retryConnection, clearRpcError, checkConnectivityNow } = useWalletContext();

    const testCorsError = () => {
        console.log("🧪 Testing CORS error...");
        const corsError = new Error("Access to fetch at 'https://public-node.testnet.rsk.co/' from origin 'http://localhost:4173' has been blocked by CORS policy: Response to preflight request doesn't pass access control check: It does not have HTTP ok status.");
    ...
    const testTimeoutError = () => {
        ...
        const timeoutError = new Error("Request timeout - network connection is slow");
    ...
    const testRealRpcCall = async () => {
        console.log("🧪 Testing real RPC call failure...");
        try {
            // This will fail if there's no internet
            const response = await fetch('https://public-node.testnet.rsk.co/', {
                method: 'POST',
            ...
            console.log("RPC call succeeded:", response.status);
        } catch (error) {
    ...
    const forceShowAlert = () => {
        console.log("🧪 Force showing alert...");
    ...
    return (
        <div style={{ 
            ...
            <div style={{ fontSize: '12px', marginBottom: '8px', color: rpcError.hasError ? 'red' : 'green' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <Button size="small" onClick={testRealRpcCall}>
                    Test Real RPC Call
                </Button>
                <Button size="small" onClick={forceShowAlert} style={{ background: 'orange', color: 'white' }}>
                    FORCE SHOW ALERT
                </Button>
                <Button size="small" onClick={testConnectivityCheck} style={{ background: 'blue', color: 'white' }}>
                    Check Connectivity
                </Button>
                <Button size="small" onClick={clearRpcError} disabled={!rpcError.hasError}>
                    Clear Error
                </Button>
            </div>
        </div>
