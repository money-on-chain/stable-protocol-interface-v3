...
export default function WalletProviders({ onCloseModal }: ProvidersProps) {
  ...
  const baseAllowed = React.useMemo(
    () => connectors.filter((c) => isInjected(c) || isWalletConnect(c) || isSafe(c)),
    [connectors]
  )
  ...
  const visibleConnectors = React.useMemo(() => {
    if (isMobileUA && !isInAppWallet) return baseAllowed.filter(isWalletConnect)
    return baseAllowed
  ...
  const wcOnly = visibleConnectors.length > 0 && visibleConnectors.every((c) => c.id === "walletConnect")

  const sortByOrder = (a: Connector, b: Connector) => {
    const key = (c: Connector) =>
      isInjected(c) ? "injected" : isWalletConnect(c) ? "walletConnect" : isSafe(c) ? "safe" : "walletConnect"
    return ORDER.indexOf(key(a) as typeof ORDER[number]) - ORDER.indexOf(key(b) as typeof ORDER[number])
  }
  ...
  const handleConnect = async (connector: Connector) => {
    try {
      ...
      notification.error({
        ...
        description: error?.shortMessage || error?.message || "Check your wallet and try again.",
      })
    } finally {
      setLoadingId(null)
    }
